...
class CoreShipmentApiTest(TestCase):
    def setUp(self):
        # available AWB
        ...
        # initial bag
        Bag.objects.create(number="BAG1234", capacity=3.0)
    ...
    def TestBagging(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(content.get('success'), 0)
        self.assertEqual(content.get('awbbefore'), ['SD', 'SD'])
    ...
    def test_case(self):
        self.TestShipmentDetailEntry()
        self.TestShipmentDetailEntry2()
        self.TestBagging()
        self.TestOutbound()
        self.TestUplifting()
        self.TestInbound()
...
class AgentShipmentApiTest(TestCase):
    def setUp(self):
        ...
        # initial bag
        Bag.objects.create(number="BAG1234")
        ...
        # initial Transportation
        Transportation.objects.create(identifier='Truck123', base='Jakarta', operator='Trucker', capacity='10', destination_id=2, origin_city_id=1, transportation_type_id=1)
        Transportation.objects.create(identifier="Flight123", base="Jakarta", operator="Flighter", capacity="30", destination_id=2, origin_city_id=1, transportation_type_id=2)
...
class CoreMultipleShipmentApiTest(TestCase):
    def setUp(self):
        # available AWB
        AWB.objects.create(number="CGK1234567890")
        AWB.objects.create(number="CGK1234567891")
    ...
    def TestOutbound(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(content.get('success'), 0)
        self.assertEqual(content.get('awbbefore'), ['PR', 'PR', 'SD', 'SD', 'SD'])
